{
  description = "mitchty.github.com";
  ...
  outputs =
    ...
    inputs.flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # Cargo artifacts for WASM builds
            ...
            wasmBuild = craneLibWasm.buildPackage (
              ...
              // {
                ...
                installPhase = ''
                  runHook preInstall
                  mkdir -p $out
                  cp -r target/wasm32-unknown-unknown/wasm-release $out/
                  runHook postInstall
                '';
              }
            );
          ...
          pkgsWasm.runCommand "mitchty-wasm-bindgen"
            ...
            ''
              ...
              ${wasmBindgenCli}/bin/wasm-bindgen \
                ...
                --target web \
                --no-typescript \
                ${wasmBuild}/wasm-release/mitchty.wasm
      ...
      in
      {
        ...
        packages = {
          inherit mitchty mitchty-lto mitchty-wasm;
          default = mitchty;
        ...
        // lib.optionalAttrs pkgs.stdenv.isLinux {
          ...
          int = self.checks.${system}.mitchty-integration-all;
        }
